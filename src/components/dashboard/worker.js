...
import {playRingSound} from '../../reducers/chat'
const FakeUserListItem = ({
  ...
  return (
    <List.Item active={isActive} onClick={(e) => selectChatFakeUser(item.id)}>
      {
      ...
      <List.Content floated="right">
        <Label color={item.unread_messages_count > 0 ? "yellow" : "grey"}>
  ...
  const getAttachmentImage = (singleChat, userId, msg) => {
    ...
    if (singleChat != null) {
      ...
      if (singleChat.attachments != null && singleChat.attachments.length > 0) {
        ...
        const id = singleChat.attachments[0].id;
...
export default class WorkerDashboard extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.chat.currentFakeUsers.length > 0 &&
      prevProps.chat.currentFakeUsers.length === 0
    ) {
      this.props.getAllDialogue(
        this.props.chat.currentFakeUsers.map((user) => user.id)
      );
    }
